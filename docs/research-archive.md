...
The live derivational target after the benchmark package is now the absolute-amplitude/polarity origin note below the recorded orbit-shape/modulus burden-fixing note, primitive-class setup note, primitive-class amplitude-lift note, and amplitude-sector origin note.
...
The next active work is no longer another archive or presentation edit, it is no longer the direct-vs-setup decision, it is no longer the first direct derivational `.tex` manuscript below the setup layer, and it is no longer the amplitude-sector origin manuscript below the amplitude-lift layer. Those steps are already recorded. The next active work is the absolute-amplitude/polarity origin manuscript below the amplitude-sector layer.

- Use [aether_flow_substrate_higher_derivative_nonlocal_bridge_self_response_off_shell_profile_selection_bridge_order_orbit_shape_modulus_primitive_class_amplitude_sector_origin_note.tex](../manuscripts/active/tex/aether_flow_substrate_higher_derivative_nonlocal_bridge_self_response_off_shell_profile_selection_bridge_order_orbit_shape_modulus_primitive_class_amplitude_sector_origin_note.tex) together with the setup and amplitude-lift notes as the launch point.
- Draft the next still-deeper manuscript that derives or sharply constrains the positive absolute amplitudes and discrete polarity data feeding that amplitude-sector note while preserving the same downstream chain.
