...
6. The current live frontier: the smaller completion-state shell-trace core is now itself forced canonically by completion-balance ambient dynamics in [aether_flow_primitive_reservoir_observer_reduction_minimal_completion_state_shell_trace_core_necessity_uniqueness_from_completion_balance_ambient_dynamics_theorem.tex](../manuscripts/active/tex/aether_flow_primitive_reservoir_observer_reduction_minimal_completion_state_shell_trace_core_necessity_uniqueness_from_completion_balance_ambient_dynamics_theorem.tex). At current scope, restating the trace core, restating the full completion-core presentation, restating the exact-shell affine nucleus, or adding another same-package ambient relay that leaves the ambient package unchanged no longer counts as the honest next benchmark-facing move.

## Current Primary Burden

The next primary manuscript should now act one level below the current ambient-dynamics frontier. It should:

- first try to derive or uniquely force the completion-balance ambient dynamics package itself from still more primitive explicit substrate structure in a way that changes benchmark-facing status
- or, if that cannot be done cleanly, prove a genuinely smaller theorem-level collapse or a sharper necessity / uniqueness result strictly inside that ambient package while preserving the same benchmark one-metric package and claim boundary
- and not default to restating the trace core, restating the full completion-core presentation, restating the exact-shell affine nucleus, or inserting another same-package ambient relay that merely preserves that same ambient package, because none of those now changes benchmark-facing status
...
The orbit-shape / primitive-class / midpoint-selector sequence remains recorded side work. It is useful and disciplined, but it is not the default next GR-derivation step unless it changes the primary benchmark-facing burden.
...
- [Ambient trace-core forcing frontier PDF](../manuscripts/active/pdf/aether_flow_primitive_reservoir_observer_reduction_minimal_completion_state_shell_trace_core_necessity_uniqueness_from_completion_balance_ambient_dynamics_theorem.pdf)
- [Ambient-dynamics next-benchmark criterion PDF](../manuscripts/active/pdf/aether_flow_primitive_reservoir_observer_reduction_completion_balance_ambient_dynamics_next_benchmark_criterion.pdf)
- [Research plan](../RESEARCH_PLAN.md)

## Archive Principle

The detailed step-by-step chronology is intentionally not duplicated here. For older sequence detail, use the manuscript tree, manuscript conclusions, and git history.
