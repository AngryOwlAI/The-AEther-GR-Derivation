...
The immediate next primary manuscript target is no longer the deeper completion-kernel origin / selection step, it is no longer the microscopic-equilibrium deeper-origin step, and it is no longer the equilibrium-reservoir primitive-origin step either; all three are now recorded. The next primary target is a still-deeper primitive-origin note for the branch-strain / neutral-current / free-mode reservoir recorded in [aether_flow_substrate_higher_derivative_substrate_response_self_response_charge_polarization_quartic_completion_selection_principle_equilibrium_reservoir_primitive_origin_note.tex](../manuscripts/active/tex/aether_flow_substrate_higher_derivative_substrate_response_self_response_charge_polarization_quartic_completion_selection_principle_equilibrium_reservoir_primitive_origin_note.tex), or another comparably concrete observer-equation gain on the same one-metric charge-polarization line. That next step should derive or justify why the primitive branch-strain carrier is forced, why only the neutral \(J-\widehat{\mathcal B}(K)\) combination is admissible, and why the free homogeneous carrier is genuinely gapped and unloaded by \(\ResidualLift\), while re-showing that elimination still yields the same coarse reservoir, the same \(\Sigma^{\mathrm{cmp}}\) solve, the same \(\Pi_{\mathrm{free}}H=0\) outcome, and the same observer-equation closure. Stronger promotion language should wait until that deeper burden has been handled cleanly.
