...
The immediate next primary manuscript target is no longer the deeper completion-kernel origin / selection step, it is no longer the microscopic-equilibrium deeper-origin step, it is no longer the equilibrium-reservoir primitive-origin step, and it is no longer the primitive-reservoir carrier-origin step either; all four are now recorded. The next primary target is a still-deeper origin or sharper selection-principle note for the carrier-origin lift recorded in [aether_flow_substrate_higher_derivative_substrate_response_self_response_charge_polarization_quartic_completion_selection_principle_primitive_reservoir_carrier_origin_note.tex](../manuscripts/active/tex/aether_flow_substrate_higher_derivative_substrate_response_self_response_charge_polarization_quartic_completion_selection_principle_primitive_reservoir_carrier_origin_note.tex), or another comparably concrete observer-equation gain on the same one-metric charge-polarization line. That next step should derive or justify why that deeper projection/current/gap sector itself exists, while re-showing that elimination still yields the same primitive reservoir, the same microscopic-equilibrium reservoir, the same \(\Sigma^{\mathrm{cmp}}\) solve, the same \(\Pi_{\mathrm{free}}H=0\) outcome, and the same observer-equation closure. Stronger promotion language should wait until that deeper burden has been handled cleanly.
