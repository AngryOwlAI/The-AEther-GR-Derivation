# GR Research Question Reset

Updated: April 13, 2026

Status: Repository guidance note. This note does not change the scientific benchmark. It changes the framing of the active derivational objective.

## Purpose

The repository should no longer frame its top-level derivational objective as:

- `derive GR`

That framing encourages recursive depth drift, because any deeper same-output relay can look like progress if failure or impossibility is not treated as an equally valid outcome.
...
This reset does **not** change the benchmark package.

The benchmark remains:

- the exact-closure package adopts GR exactly as the operational relativistic sector with ordinary matter coupling
...
- [GR_DERIVATION_RESET_PLAN.md](GR_DERIVATION_RESET_PLAN.md)
...
- frontier promotion
- agent behavior
- repo-facing summaries of active research
...
The following is no longer an acceptable default outcome:

- `the remaining honest burden moves one layer deeper again`

That sentence may appear inside technical work, but it cannot function as the repository's main answer to the GR question.

Another same-output deeper-origin relay is not, by itself, a research verdict.
...
- `keep constructing deeper bridge manuscripts until derivation appears`

This means the agent must treat obstruction, scoped failure, or suspension as valid scientific outcomes rather than as failure to continue.

## Practical Consequence

A new main-line manuscript should count as progress only if it does at least one of the following:
...
This reset is methodological, not doctrinal.

It does not weaken the benchmark package, and it does not declare the derivation impossible.
