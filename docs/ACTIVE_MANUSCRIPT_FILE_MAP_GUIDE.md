...
- the current file map carries a single `active_primary` row at `primitive_reduction_current_pre_proto_section_hidden_response_reduced_projection_response_substrate_structure_necessity_uniqueness_frontier`, anchored on the theorem proving that once the fixed full canonical current-body image core is held fixed any benchmark-faithful reduced projection-response substrate structure is necessary and unique up to inert kernel extension, rather than on another same-output relay, inert-kernel carrier restatement, or a replay of the larger body, projection-core, or full substrate-law presentations
