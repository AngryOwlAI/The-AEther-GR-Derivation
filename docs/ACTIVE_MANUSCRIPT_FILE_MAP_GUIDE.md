...
- the current file map carries a single `active_primary` row at `primitive_reduction_current_pre_proto_section_hidden_response_current_body_image_core_canonical_minimal_visible_response_representative_frontier`, anchored on the theorem proving that even the full canonical current-body image core already collapses benchmark-facingly to the canonical minimal visible-response representative on the fixed displacement fibers, rather than on another same-output current-image-core restatement, reduced-carrier restatement, inert-kernel extension, or a replay of the larger projection/body or full substrate-law presentations
