...
- the current file map carries a single `active_primary` row at `primitive_reduction_current_pre_proto_section_hidden_response_canonical_image_response_exact_shell_necessity_uniqueness_frontier`, anchored on the theorem proving that once the fixed canonical image body and the fixed restricted projection / canonical-lift relation are held fixed the image response law and exact-shell image are uniquely forced, rather than on another same-output relay or a replay of the larger body, projection-core, or full substrate-law presentations
