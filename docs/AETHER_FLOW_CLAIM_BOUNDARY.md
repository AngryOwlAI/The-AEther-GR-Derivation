...
- The file map currently carries one live `active_primary` theorem on the primitive-reservoir observer-reduction line at seed forcing scope, but benchmark-facing status is governed by the derivation-gates note, the post-bridge status note, the upstream compression theorem, and the current seed frontier criterion.
