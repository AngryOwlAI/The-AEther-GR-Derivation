...
- Treat [aether_flow_primitive_reservoir_observer_reduction_seed_generating_substrate_germ_dynamics_collapse_to_observer_relevant_germ_seed_transport_core_theorem.tex](../manuscripts/active/tex/aether_flow_primitive_reservoir_observer_reduction_seed_generating_substrate_germ_dynamics_collapse_to_observer_relevant_germ_seed_transport_core_theorem.tex) as the single current `active_primary` row: the full germ package now collapses benchmark-facingly to a smaller observer-relevant germ seed-transport core, so the live burden no longer sits on the full germ package itself.
- The next primary gain should therefore derive or force the smaller observer-relevant germ seed-transport core below the former full germ frontier that yields the current primitive observer-reduction chain the continuity-Hessian compressed core and the benchmark one-metric observer package.
- Another same-output deeper-origin relay that merely preserves the same seed package the same root package the same foundation package the same ground package the same origin package the same precursor package the same lift package the same shell-restriction package the same full germ package the same observer-relevant germ seed-transport core or the same compressed observer-relevant core or a reopening of older screened layers is not the honest default next move.
...
- The file map currently carries one live `active_primary` theorem on the primitive-reservoir observer-reduction line at germ-core collapse scope, but benchmark-facing status is governed by the derivation-gates note, the post-bridge status note, the upstream compression theorem, and the current frontier control layer.
- The current missing bridge burden is the substrate law below the seed frontier that forces the current primitive observer-reduction chain yielding the continuity-Hessian compressed core and the benchmark one-metric observer package.
