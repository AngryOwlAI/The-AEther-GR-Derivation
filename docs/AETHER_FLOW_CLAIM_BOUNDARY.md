...
- Treat [aether_flow_primitive_reservoir_observer_reduction_precursor_generating_substrate_origin_dynamics_necessity_uniqueness_from_origin_generating_substrate_ground_dynamics_theorem.tex](../manuscripts/active/tex/aether_flow_primitive_reservoir_observer_reduction_precursor_generating_substrate_origin_dynamics_necessity_uniqueness_from_origin_generating_substrate_ground_dynamics_theorem.tex) as the single current `active_primary` row: the precursor-generating substrate origin package is now a canonical and unique output of origin-generating substrate ground dynamics, so the live burden has moved below origin scope to ground scope.
- Treat [aether_flow_primitive_reservoir_observer_reduction_precursor_generating_substrate_origin_dynamics_next_benchmark_criterion.tex](../manuscripts/active/tex/aether_flow_primitive_reservoir_observer_reduction_precursor_generating_substrate_origin_dynamics_next_benchmark_criterion.tex) as the active benchmark-facing routing note below that frontier: the next honest move is now one level below origin scope, first targeting origin-generating substrate ground dynamics and, if that cannot yet be done cleanly, a genuinely smaller theorem or sharper necessity/uniqueness result strictly inside precursor-generating substrate origin dynamics.
- The next primary gain should therefore derive or force the substrate law below the origin frontier that yields the current primitive observer-reduction chain, the continuity-Hessian compressed core, and the benchmark one-metric observer package.
- Another same-output deeper-origin relay that merely preserves the same origin package, the same precursor package, the same lift package, the same shell-restriction package, or the same compressed observer-relevant core, or a reopening of older screened layers, is not the honest default next move.
...
- The file map currently carries one live `active_primary` theorem on the primitive-reservoir observer-reduction line at origin forcing scope, but benchmark-facing status is governed by the derivation-gates note, the post-bridge status note, the upstream compression theorem, and the current origin frontier criterion.
- The current missing bridge burden is the substrate law below the origin frontier that forces the current primitive observer-reduction chain yielding the continuity-Hessian compressed core and the benchmark one-metric observer package.
