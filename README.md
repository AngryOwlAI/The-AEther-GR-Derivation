...
This project should therefore not be read as claiming that GR has already been derived from first principles. Its present position is narrower and more disciplined: the exact-closure package is the benchmark theory statement already in hand, while the derivational manuscripts record the active attempt to derive that benchmark from deeper substrate dynamics. As of March 27, 2026, the new benchmark-facing step just recorded is the carrier-origin precursor-selection note [aether_flow_substrate_higher_derivative_substrate_response_self_response_charge_polarization_quartic_completion_selection_principle_primitive_reservoir_carrier_origin_precursor_selection_note.tex](manuscripts/active/tex/aether_flow_substrate_higher_derivative_substrate_response_self_response_charge_polarization_quartic_completion_selection_principle_primitive_reservoir_carrier_origin_precursor_selection_note.tex). At disciplined precursor-selection scope, that note derives the previously recorded carrier-origin projection/current/gap sector itself as the low-selection stationary image of a still-deeper precursor layer: the carrier-origin tensor seed survives only as the projected readout into \(\CarrierClass\), the relabeling seed survives only as the projected current into \(T^*\Sub\), the homogeneous seed survives only as the projected free readout into \(\mathscr C_{\mathrm{cmp}}^{\mathrm{free}}\), and the orthogonal precursor complements are positively gapped spectators. Eliminating that deeper layer reproduces the same carrier-origin functional and hence the same primitive reservoir, the same microscopic-equilibrium reservoir, the same \(\Pi_{\mathrm{free}}H=0\) outcome, the same \(\Sigma^{\mathrm{cmp}}\) solve, and the same one operative metric while preserving the recorded Phase D / E and Phase F gains. The next benchmark-facing step is now one layer below that new precursor-selection note: derive or justify a still-deeper origin or sharper selection principle for the precursor-selection lift itself, or else produce another comparably concrete observer-equation gain on the same one-metric charge-polarization line.
