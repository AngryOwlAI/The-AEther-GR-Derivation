...
The immediate working next step is now manuscript production rather than another repository-presentation pass. The setup note, amplitude-lift note, amplitude-sector note, absolute-amplitude/polarity note, and midpoint/fiber note now form the recorded launch stack below the burden-fixing layer. The midpoint/fiber note isolates midpoint selection as the residual freedom once the oriented absolute-amplitude data are fixed. The next new `.tex` manuscript should therefore be a midpoint-tuple origin or selection note below that midpoint/fiber layer. That manuscript should derive or sharply constrain the midpoint tuple itself, or select a canonical midpoint representative, define the map into the recorded midpoint/fiber layer and hence through the already-frozen downstream maps, prove that the same raw-orbit lift and same downstream package remain unchanged, and revisit the scale-orbit or residual vacuum-angle verdicts only if genuinely new quotient or locking structure actually appears.
