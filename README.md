...
This project should therefore not be read as claiming that GR has already been derived from first principles. Its present position is narrower and more disciplined: the exact-closure package is the benchmark theory statement already in hand, while the derivational manuscripts record the active attempt to derive that benchmark from deeper substrate dynamics. As of March 27, 2026, the new benchmark-facing step just recorded is the exact-preservation normal-form origin note [aether_flow_substrate_higher_derivative_substrate_response_self_response_charge_polarization_quartic_completion_selection_principle_primitive_reservoir_carrier_origin_precursor_selection_exact_preservation_normal_form_origin_note.tex](manuscripts/active/tex/aether_flow_substrate_higher_derivative_substrate_response_self_response_charge_polarization_quartic_completion_selection_principle_primitive_reservoir_carrier_origin_precursor_selection_exact_preservation_normal_form_origin_note.tex). At disciplined ambient-sector scope, that note shows that the previously recorded raw deeper readout spaces are themselves the projected unsuppressed sectors of still more primitive ambient substrate spaces, that the visible ambient maps must factor through those projections into \(\CarrierClass\), \(T^*\Sub\), and \(\mathscr C_{\mathrm{cmp}}^{\mathrm{free}}\), that the ambient complements are positively gapped exact-preserving spectators, and that eliminating this still-deeper layer reproduces the same exact-preservation normal form and hence the same carrier-origin functional, the same primitive reservoir, the same microscopic-equilibrium reservoir, the same \(\Pi_{\mathrm{free}}H=0\) outcome, the same \(\Sigma^{\mathrm{cmp}}\) solve, and the same one operative metric while preserving the recorded Phase D / E and Phase F gains. The next benchmark-facing step is now one layer below that new exact-preservation-normal-form origin note: derive or justify why those ambient substrate spaces, their orthogonal projectors onto the raw deeper sectors, the factorized visible maps, and the positive spectator gaps themselves arise from still more explicit substrate microphysics, or else produce another comparably concrete observer-equation gain on the same one-metric charge-polarization line.
