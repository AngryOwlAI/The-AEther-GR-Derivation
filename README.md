...
This project should therefore not be read as claiming that GR has already been derived from first principles. Its present position is narrower and more disciplined: the exact-closure package is the benchmark theory statement already in hand, while the derivational manuscripts record the active attempt to derive that benchmark from deeper substrate dynamics. As of March 28, 2026, the new benchmark-facing step just recorded is the exact-preservation normal-form microphysical-Hessian deeper-origin note [aether_flow_substrate_response_charge_polarization_exact_preservation_microphysical_hessian_deeper_origin_note.tex](manuscripts/active/tex/aether_flow_substrate_response_charge_polarization_exact_preservation_microphysical_hessian_deeper_origin_note.tex). At disciplined symmetry-charge / compatibility-response scope, that note shows that the symmetry-projector / compatibility-kernel / reduced-Hessian package is itself the stationary reduced output of a still deeper layer: the recorded symmetry projectors are forced as Haar-average fixed-point projectors of compact exact-preserving symmetry actions, the recorded compatibility sectors are forced as zero-current kernels of compatibility-current maps, and the recorded reduced Hessians are forced as compressed primitive response operators with the same coercive positive gaps. Eliminating that deeper layer still reproduces the same exact-preservation normal form and hence the same carrier-origin functional, the same primitive reservoir, the same microscopic-equilibrium reservoir, the same \(\Pi_{\mathrm{free}}H=0\) outcome, the same \(\Sigma^{\mathrm{cmp}}\) solve, and the same one operative metric while preserving the recorded Phase D / E and Phase F gains. The next benchmark-facing step is therefore one layer below that new deeper-origin note: derive or justify why those compact symmetry actions, compatibility-current maps, and primitive response operators themselves are forced by yet more primitive substrate structure, or else produce another comparably concrete observer-equation gain on the same one-metric charge-polarization line.
