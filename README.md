...
The immediate working next step is now manuscript production rather than another repository-presentation pass. The setup note, amplitude-lift note, amplitude-sector note, and absolute-amplitude/polarity note now form the recorded launch stack below the burden-fixing layer. The next new `.tex` manuscript should therefore be a midpoint/fiber origin or selection note below the absolute-amplitude/polarity layer. That manuscript should derive or sharply constrain the midpoint/fiber data, or select a canonical representative, for the unequal positive-pair sector, define the map into the recorded absolute-amplitude/polarity layer and hence through the already-frozen downstream maps, prove that the same raw-orbit lift and same downstream package remain unchanged, and revisit the scale-orbit or residual vacuum-angle verdicts only if genuinely new quotient or locking structure actually appears.
