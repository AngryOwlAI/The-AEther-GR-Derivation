...
This project should therefore not be read as claiming that GR has already been derived from first principles. Its present position is narrower and more disciplined: the exact-closure package is the benchmark theory statement already in hand, while the derivational manuscripts record the active attempt to derive that benchmark from deeper substrate dynamics. As of March 27, 2026, the new benchmark-facing step just recorded is the carrier-origin precursor-selection deeper-origin note [aether_flow_substrate_higher_derivative_substrate_response_self_response_charge_polarization_quartic_completion_selection_principle_primitive_reservoir_carrier_origin_precursor_selection_deeper_origin_note.tex](manuscripts/active/tex/aether_flow_substrate_higher_derivative_substrate_response_self_response_charge_polarization_quartic_completion_selection_principle_primitive_reservoir_carrier_origin_precursor_selection_deeper_origin_note.tex). At disciplined exact-preservation normal-form scope, that note shows that the precursor-selection structure is itself forced: any still-deeper sector that preserves the same carrier-origin functional must read out into \(\CarrierClass\), \(T^*\Sub\), and \(\mathscr C_{\mathrm{cmp}}^{\mathrm{free}}\), the old precursor spaces are recovered as the image-plus-kernel normal forms of those deeper readout maps, the orthogonal precursor complements are exactly the positively gapped kernel sectors, and eliminating that deeper layer reproduces the same carrier-origin functional and hence the same primitive reservoir, the same microscopic-equilibrium reservoir, the same \(\Pi_{\mathrm{free}}H=0\) outcome, the same \(\Sigma^{\mathrm{cmp}}\) solve, and the same one operative metric while preserving the recorded Phase D / E and Phase F gains. The next benchmark-facing step is now one layer below that new deeper-origin note: derive or justify why that exact-preservation normal form itself arises from still more explicit substrate microphysics, or else produce another comparably concrete observer-equation gain on the same one-metric charge-polarization line.
