...
- [ ] Treat the bridge-entry and bridge-object derivations already on record, especially the continuity-Hessian core derivation theorem, datum-origin theorem, defect-fiber factorization theorem, one-metric / ordinary-matter output-map theorem, chain-forcing theorem, selector-existence theorem, stationary-construction theorem, explicit-package necessity / uniqueness theorem, primitive reversible constrained dynamics necessity / uniqueness from ambient-shell law theorem, the exact-shell affine nucleus collapse theorem, the direct exact-shell affine nucleus forcing theorem from minimal zero-response affine completion core, and the later completion-core trace-collapse theorem, as the real benchmark-facing gains already in hand.
- [ ] Treat the single current `active_primary` row in `docs/ACTIVE_MANUSCRIPT_FILE_MAP.csv` as the live completion-core trace-collapse frontier of the primitive-reservoir observer-reduction line, not as a completed GR derivation.
- [ ] Treat the current bridge-object gain as the theorem that the full minimal zero-response affine completion-core presentation collapses to the smaller completion-state shell-trace core while still yielding the continuity-Hessian compressed core and the benchmark one-metric observer package.
- [ ] The next primary manuscript must either:
  - derive or uniquely force that trace core from still more primitive substrate structure in a way that changes benchmark-facing status
  - prove a genuinely smaller theorem-level collapse, sharper necessity result, or sharper uniqueness result inside that trace core while preserving the same benchmark one-metric package and claim boundary
...
- [ ] Do not spend the move on restating the full completion-core graph presentation, restating the exact-shell affine nucleus, re-expanding the larger primitive reversible constrained dynamics package, inserting another same-output deeper-origin relay that preserves the same trace-core consequence, or reopening preserved deeper ambient relays or older screened layers.
...
- [x] The file map currently carries one live `active_primary` row for the completion-core trace-collapse frontier rather than for the older completion-core or nucleus frontiers.
- [x] The prior completion-core frontier routing note is preserved as handoff history, and the live trace-core frontier routing note is recorded with screening against completion-core restatements, nucleus restatements, preserved deeper ambient relays, and same-output deeper relays that preserve the same trace-core consequence.
