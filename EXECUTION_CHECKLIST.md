...
- [ ] Treat the single current `active_primary` row in `docs/ACTIVE_MANUSCRIPT_FILE_MAP.csv` as the live shell-restriction forcing frontier of the primitive-reservoir observer-reduction line, not as a completed GR derivation.
- [ ] Treat the current bridge-side gain as the theorem that the split zero-hidden selector-shell core itself is forced by primitive substrate shell restriction while still yielding the same selector-law consequence, the same continuity-Hessian compressed core, and the benchmark one-metric observer package.
- [ ] The next primary manuscript should therefore go one level below the current shell-restriction frontier: first try to derive or uniquely force the shell-generating substrate lift dynamics from still more primitive explicit substrate structure in a way that actually changes benchmark-facing status while preserving the same benchmark one-metric package and claim boundary.
- [ ] If that cannot be done cleanly, the fallback is to prove a genuinely smaller theorem-level collapse, sharper necessity result, or sharper uniqueness result strictly inside primitive substrate shell restriction while preserving the same benchmark one-metric package and claim boundary.
...
- [x] The file map currently carries one live `active_primary` row for the shell-restriction forcing frontier rather than for the older selector, split-core, ambient, trace-core, completion-core, or nucleus frontiers.
- [x] The prior split-core frontier routing note is preserved as handoff history, and the live shell-restriction frontier routing note is recorded with screening against split-core restatements, selector-law restatements, combined-response restatements, ambient-package restatements, trace-core restatements, completion-core restatements, exact-shell affine nucleus restatements, and benchmark-neutral shell-restriction relays that leave the shell-restriction package unchanged.
