...
- [ ] Treat the single current `active_primary` row in `docs/ACTIVE_MANUSCRIPT_FILE_MAP.csv` as the live seed-from-germ forcing frontier of the primitive-reservoir observer-reduction line, not as a completed GR derivation.
- [ ] Treat the current bridge-side gain as the theorem that the root-generating substrate seed package itself is forced by seed-generating substrate germ dynamics while still yielding the same root consequence, the same continuity-Hessian compressed core, and the benchmark one-metric observer package.
- [ ] The next primary manuscript should therefore go one level below the current seed frontier: first try to derive or uniquely force the seed-generating substrate germ dynamics from still more primitive explicit substrate structure in a way that actually changes benchmark-facing status while preserving the same benchmark one-metric package and claim boundary.
- [ ] If that cannot be done cleanly, the fallback is to prove a genuinely smaller theorem-level collapse, sharper necessity result, or sharper uniqueness result strictly inside seed-generating substrate germ dynamics while preserving the same benchmark one-metric package and claim boundary.
...
- [ ] Do not spend the move on restating the split selector-shell core, restating the full selector law, restating the combined response law, restating the ambient package, restating the trace core, restating the full completion-core presentation, restating the exact-shell affine nucleus, adding another foundation relay, adding another root relay that preserves the same root package without shrinking or forcing it further, adding another seed relay that preserves the same seed package without shrinking or forcing it further, adding another germ relay that preserves the same germ package without shrinking or forcing it further, or reopening older screened layers.
...
- [x] The file map currently carries one live `active_primary` row for the seed-from-germ forcing frontier rather than for the older root, shell-restriction, lift, precursor, split-core, ambient, trace-core, completion-core, or nucleus frontiers.
- [x] The prior root frontier routing note is preserved as handoff history, and the live seed frontier routing note is recorded with screening against seed-package restatements, root-package restatements, foundation-package restatements, ground-package restatements, origin-package restatements, precursor-package restatements, lift-package restatements, shell-restriction restatements, split-core restatements, selector-law restatements, combined-response restatements, ambient-package restatements, trace-core restatements, completion-core restatements, exact-shell affine nucleus restatements, and benchmark-neutral seed or germ relays that leave the current packages unchanged.
