...
- [ ] Treat the single current `active_primary` row in `docs/ACTIVE_MANUSCRIPT_FILE_MAP.csv` as the live observer-relevant pullback-quadratic-core frontier inside section-centered hidden-response scope on the primitive-reservoir observer-reduction line, not as a completed GR derivation.
- [ ] Treat the current bridge-side gain as the theorem that the full current section-centered hidden-response package collapses benchmark-facingly to a smaller observer-relevant pullback-quadratic core while still yielding the same current residual-normal-form datum, the same benchmark-faithful pre-proto-section dynamics package, the same continuity-Hessian compressed core, and the benchmark one-metric observer package.
- [ ] The next primary manuscript should therefore go one level below the current observer-relevant pullback-quadratic-core frontier: first try to derive or justify that pullback core from still more primitive explicit substrate structure in a way that actually changes benchmark-facing status while preserving the same benchmark one-metric package, the same ordinary-matter route, and the same claim boundary.
- [ ] If that cannot be done cleanly, the fallback is to prove a genuinely smaller theorem-level collapse, sharper necessity result, or sharper uniqueness result strictly inside that pullback core while preserving the same benchmark one-metric package, the same ordinary-matter route, and the same claim boundary.
...
- [x] The file map currently carries one live `active_primary` row for the observer-relevant pullback-quadratic-core frontier inside section-centered hidden-response scope rather than for the older hidden-response-package, residual-normal-form, pre-proto-section-package, minimizer-section-core, minimizer-image-core, root, shell-restriction, lift, precursor, split-core, ambient, trace-core, completion-core, or nucleus frontiers.
- [x] The prior hidden-response frontier is preserved as handoff history, and the current live frontier is the smaller pullback-quadratic core rather than the larger hidden-response package itself.
