...
- [ ] Treat the single current `active_primary` row in `docs/ACTIVE_MANUSCRIPT_FILE_MAP.csv` as the live precursor-dynamics forcing frontier of the primitive-reservoir observer-reduction line, not as a completed GR derivation.
- [ ] Treat the current bridge-side gain as the theorem that the lift-generating substrate precursor package itself is forced by precursor-generating substrate origin dynamics while still yielding the same lift consequence, the same continuity-Hessian compressed core, and the benchmark one-metric observer package.
- [ ] The next primary manuscript should therefore go one level below the current precursor frontier: first try to derive or uniquely force the precursor-generating substrate origin dynamics from still more primitive explicit substrate structure in a way that actually changes benchmark-facing status while preserving the same benchmark one-metric package and claim boundary.
- [ ] If that cannot be done cleanly, the fallback is to prove a genuinely smaller theorem-level collapse, sharper necessity result, or sharper uniqueness result strictly inside lift-generating substrate precursor dynamics while preserving the same benchmark one-metric package and claim boundary.
...
- [x] The file map currently carries one live `active_primary` row for the precursor-forcing frontier rather than for the older shell-restriction, lift, split-core, ambient, trace-core, completion-core, or nucleus frontiers.
- [x] The prior lift frontier routing note is preserved as handoff history, and the live precursor frontier routing note is recorded with screening against lift-package restatements, shell-restriction restatements, split-core restatements, selector-law restatements, combined-response restatements, ambient-package restatements, trace-core restatements, completion-core restatements, exact-shell affine nucleus restatements, and benchmark-neutral precursor relays that leave the precursor package unchanged.
