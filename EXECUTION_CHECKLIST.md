...
- [ ] Treat the single current `active_primary` row in `docs/ACTIVE_MANUSCRIPT_FILE_MAP.csv` as the live germ-core collapse frontier of the primitive-reservoir observer-reduction line, not as a completed GR derivation.
- [ ] Treat the current bridge-side gain as the theorem that the full seed-generating substrate germ package collapses to a smaller observer-relevant germ seed-transport core while still yielding the same seed consequence, the same continuity-Hessian compressed core, and the benchmark one-metric observer package.
- [ ] The next primary manuscript should therefore go one level below the current germ-core frontier: first try to derive or uniquely force the observer-relevant germ seed-transport core from still more primitive explicit substrate structure in a way that actually changes benchmark-facing status while preserving the same benchmark one-metric package and claim boundary.
- [ ] If that cannot be done cleanly, the fallback is to prove a genuinely smaller theorem-level collapse, sharper necessity result, or sharper uniqueness result strictly inside that core while preserving the same benchmark one-metric package and claim boundary.
...
- [ ] Do not spend the move on restating the split selector-shell core, restating the full selector law, restating the combined response law, restating the ambient package, restating the trace core, restating the full completion-core presentation, restating the exact-shell affine nucleus, adding another foundation relay, adding another root relay that preserves the same root package without shrinking or forcing it further, adding another seed relay that preserves the same seed package without shrinking or forcing it further, adding another full-germ relay that preserves the same observer-relevant germ seed-transport core without shrinking or forcing it further, or reopening older screened layers.
...
- [x] The file map currently carries one live `active_primary` row for the germ-core collapse frontier rather than for the older seed-forcing, root, shell-restriction, lift, precursor, split-core, ambient, trace-core, completion-core, or nucleus frontiers.
- [x] The prior seed-frontier routing note is preserved as handoff history, and the current live frontier is the smaller observer-relevant germ seed-transport core rather than the full germ package.
