...
- [ ] Treat the single current `active_primary` row in `docs/ACTIVE_MANUSCRIPT_FILE_MAP.csv` as the live pre-proto-section minimizer-section-core frontier of the primitive-reservoir observer-reduction line, not as a completed GR derivation.
- [ ] Treat the current bridge-side gain as the theorem that the observer-relevant pre-proto-section minimizer-image core collapses to a smaller observer-relevant pre-proto-section minimizer-section core while still yielding the same continuity-Hessian compressed core and the benchmark one-metric observer package.
- [ ] The next primary manuscript should therefore go one level below the current pre-proto-section minimizer-section-core frontier: first try to derive or justify the observer-relevant pre-proto-section minimizer-section core from still more primitive explicit substrate structure in a way that actually changes benchmark-facing status while preserving the same benchmark one-metric package, the same ordinary-matter route, and the same claim boundary.
- [ ] If that cannot be done cleanly, the fallback is to prove a genuinely smaller theorem-level collapse, sharper necessity result, or sharper uniqueness result strictly inside that section core while preserving the same benchmark one-metric package, the same ordinary-matter route, and the same claim boundary.
...
- [x] The file map currently carries one live `active_primary` row for the pre-proto-section minimizer-section-core frontier rather than for the older proto-section-package, pre-proto-section-package, minimizer-image-core, root, shell-restriction, lift, precursor, split-core, ambient, trace-core, completion-core, or nucleus frontiers.
- [x] The prior pre-proto-section minimizer-image-core frontier is preserved as handoff history, and the current live frontier is the smaller observer-relevant pre-proto-section minimizer-section core rather than the larger image-core presentation.
