...
- [ ] The next primary manuscript should first try to derive or uniquely force the completion-balance ambient dynamics package itself from still more primitive substrate structure in a way that changes benchmark-facing status while preserving the same benchmark one-metric package and claim boundary.
...
- [ ] Do not spend the move on restating the completion-state shell-trace core, restating the full completion-core graph presentation, restating the exact-shell affine nucleus, replaying preserved ambient-style relays that preserve the same ambient package without shrinking or forcing it further, re-expanding the larger primitive reversible constrained dynamics package, or reopening older screened layers.
