...
- [ ] Treat the bridge-entry and bridge-object derivations already on record, especially the continuity-Hessian core derivation theorem, datum-origin theorem, defect-fiber factorization theorem, one-metric / ordinary-matter output-map theorem, chain-forcing theorem, selector-existence theorem, stationary-construction theorem, explicit-package necessity / uniqueness theorem, primitive reversible constrained dynamics necessity / uniqueness from ambient-shell law theorem, the exact-shell affine nucleus collapse theorem, the direct exact-shell affine nucleus forcing theorem from minimal zero-response affine completion core, the later completion-core trace-collapse theorem, and the direct ambient trace-core forcing theorem, as the real benchmark-facing gains already in hand.
- [ ] Treat the single current `active_primary` row in `docs/ACTIVE_MANUSCRIPT_FILE_MAP.csv` as the live trace-core ambient-forcing frontier of the primitive-reservoir observer-reduction line, not as a completed GR derivation.
- [ ] Treat the current bridge-object gain as the theorem that the smaller completion-state shell-trace core itself is forced by completion-balance ambient dynamics while still yielding the continuity-Hessian compressed core and the benchmark one-metric observer package.
- [ ] The next primary manuscript should therefore go one level below the current ambient-dynamics frontier: first try to derive or uniquely force the completion-balance ambient dynamics package itself from still more primitive substrate structure in a way that actually changes benchmark-facing status while preserving the same benchmark one-metric package and claim boundary.
- [ ] If that cannot be done cleanly, the fallback is to prove a genuinely smaller theorem-level collapse, sharper necessity result, or sharper uniqueness result strictly inside that ambient package while preserving the same benchmark one-metric package and claim boundary.
...
- [ ] Do not spend the move on restating the completion-state shell-trace core, restating the full completion-core graph presentation, restating the exact-shell affine nucleus, adding another ambient relay that preserves the same ambient package without shrinking or forcing it further, re-expanding the larger primitive reversible constrained dynamics package, or reopening older screened layers.
...
- [x] The file map currently carries one live `active_primary` row for the trace-core ambient-forcing frontier rather than for the older trace-core, completion-core, or nucleus frontiers.
- [x] The prior trace-core frontier routing note is preserved as handoff history, and the live ambient-dynamics frontier routing note is recorded with screening against trace-core restatements, completion-core restatements, nucleus restatements, and benchmark-neutral ambient relays that leave the ambient package unchanged.
