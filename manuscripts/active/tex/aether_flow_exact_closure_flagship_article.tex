...
The immediate public task is therefore not another bounded bridge installment on the frozen current line. It is to package, review, and present the exact-closure sequence as the achieved flagship result. That stance depends on two fixed facts already recorded in the active sequence: the benchmark package is the positive exact-theory statement already in hand, and the current bounded primitive-reservoir line has already ended in the scoped verdict \texttt{Not Derived On Current Line} on its frozen bridge object.
...
\begin{enumerate}
    \item \textbf{Standard GR with interpretive language.} Relative to ordinary GR, the active benchmark package keeps the same low-energy equations and observable content. The difference is not a second field equation, a new low-energy graviton sector, or a modified matter route. It is the disciplined interpretive overlay supplied by the \Aether{} / \Aflow{} ontology and by the congruence-based geometric reading of that ontology.
    \item \textbf{Einstein-aether theory.} Einstein-aether theory introduces a dynamical unit timelike vector field at observer level and therefore a genuine preferred-frame sector with additional coupled propagating modes (Jacobson and Mattingly, 2001; Jacobson and Mattingly, 2004). The active benchmark package does not do that. It claims no surviving observer-level aether vector field, no extra low-energy graviton sector, and no modified Einsteinian observer law.
    \item \textbf{Thermodynamic or other emergent-gravity derivations.} Programs such as Jacobson's derivation of the Einstein equation as an equation of state (Jacobson, 1995) are closer in ambition to the downstream derivation program than to the benchmark package itself. Their category is a derivational explanation of GR, not an exact-GR interpretive package already separated from its open derivation burden.
    \item \textbf{Sakharov-style induced gravity.} Sakharov-style induced-gravity proposals likewise ask how the Einstein-Hilbert sector may arise effectively from deeper microphysics (Sakharov, 1967/1991 English reprint). That is conceptually adjacent to the project's open foundational ambition, but it is not the same as claiming that such an induced derivation has already been completed here.
    \item \textbf{Ho\v{r}ava gravity and other preferred-foliation alternatives.} Ho\v{r}ava gravity proposes a different dynamical framework with preferred foliation structure and altered mode content, even when it is designed to approach relativistic behavior in an infrared limit (Ho\v{r}ava, 2009). The active benchmark package is not of that type, because its operational content is exactly GR rather than a modified low-energy gravity law awaiting experimental separation from GR.
\end{enumerate}
...
\begin{remark}
Representative comparison points are Einstein's formulation of GR (1915/1916), Einstein-aether theory (Jacobson and Mattingly, 2001; Jacobson and Mattingly, 2004), Jacobson's thermodynamic derivation of the Einstein equation (1995), Sakharov-style induced gravity (1967/1991 English reprint), and Ho\v{r}ava gravity (2009).
\end{remark}
...
\begin{remark}
If a future bridge route succeeds, it must recover the same exact relativistic benchmark already fixed by the flagship package. If a future bridge route fails, the project contracts back to the exact-closure theory rather than back to ontology alone. At the current stage, the bounded primitive-reservoir line has already ended in a scoped no-go on its frozen bridge object, so any resumed derivational continuation must begin on a genuinely new line with an explicit observer-localizing law or equivalent new structural principle. At minimum, that new law must say how substrate structure maps to observer spacetime fields, how the operative metric is generated locally, and how the Einsteinian observer equation together with relativistic recovery arise from that same local structure.
\end{remark}
