...
\newcommand{\ExactShellCore}{\mathfrak R^{\mathrm{core}}}
...
The active derivational program of \TheoryName{} now has an explicit safeguard against recursive depth drift. The benchmark exact-closure package is fixed, the same-package observer-side remainder gate has already been settled on the active completed branch, and the primitive-reservoir observer-reduction line has already sharpened the pre-nucleus frontier to the observer-relevant exact-shell core \(\ExactShellCore\) and then moved the classical main line below that core by deriving it from deeper pre-core dynamics. The next honest burden is therefore not ``something deeper'' in the abstract. It is benchmark facing only if it recovers that same exact-shell core, proves that the core itself is still not minimal, or otherwise closes a still-open derivation gate.
...
\begin{enumerate}
    ...
    \item The exact-shell-core collapse theorem, which showed that the full pre-nucleus package contains benchmark-neutral ambient presentation data and that the benchmark-relevant burden at that stage factors through the observer-relevant exact-shell core \(\ExactShellCore\).
    \item The current active-primary continuation, namely the theorem that derives that same exact-shell core \(\ExactShellCore\) from exact-shell-core-generating substrate pre-core dynamics while preserving the same benchmark one-metric package.
\end{enumerate}
\end{definition}

\begin{remark}
These inputs already determine the current burden. They do not require a quantum ingredient by name. They require a benchmark-faithful derivational gain answerable to the benchmark-relevant exact-shell core \(\ExactShellCore\) and its allowed continuations.
\end{remark}
...
\begin{enumerate}
    \item It gives an explicit map from the quantum package \(\QuantumPackage\) to the current benchmark-relevant exact-shell core \(\ExactShellCore\), or to a strictly smaller benchmark-relevant datum inside that core.
...
At the current frontier, an explicitly quantum reformulation does not count as primary progress merely because it replaces classical notation by Hilbert-space or operator notation while preserving the same unresolved exact-shell-core burden.
\end{proposition}

\begin{proof}
The current routing rule already blocks a same-output deeper-origin relay from counting as new front-line progress when it preserves the same benchmark one-metric package and only relocates the unexplained substrate layer deeper. Replacing a classical relay by a quantum relay without deriving the current benchmark-relevant exact-shell core, collapsing it further, or discharging a benchmark derivation gate does not alter that logic. It changes the vocabulary of the deeper layer without changing the benchmark-facing burden. Therefore it does not by itself change project status.
\end{proof}
...
\begin{enumerate}
    \item it derives or justifies the observer-relevant pre-nucleus exact-shell core \(\ExactShellCore\) from a more primitive benchmark-faithful quantum package;
    \item it proves a genuinely smaller theorem-level benchmark-relevant collapse strictly inside the current exact-shell core;
...
\begin{proof}
The fixed routing inputs already determine that the benchmark-relevant target for any replacement branch is the exact-shell core \(\ExactShellCore\) and that the classical main line has already moved below it. A replacement branch must therefore do benchmark-facing work at that same target or deeper. Item (1) does so directly by deriving the same exact-shell core from a more primitive quantum structure. Item (2) does so by proving that even that current core is not yet minimal. Item (3) covers the only other honest way a quantum branch could matter at current scope: it might discharge a still-open benchmark derivation gate by a route not already available on the active line. Absent one of those gains, the branch does not change the benchmark-facing burden and is therefore screened by the repository rule against recursive depth drift.
\end{proof}
