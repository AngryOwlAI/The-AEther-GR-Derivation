...
\newcommand{\PreNucPackage}{\mathfrak P^{\mathrm{pre\mbox{-}nuc}}}
...
This note records a conservative answer. An explicitly quantum branch is not nonsense. It is a legitimate possibility whenever the deeper substrate law is suspected to be amplitude-level, operator-level, or intrinsically nonclassical. But it is not automatically the missing bridge either. At current scope, the repository already has a fixed benchmark exact-closure package and a current active-primary frontier below the proto-anchored exact-shell transport nucleus. A quantum branch should therefore count as primary only if it does benchmark-facing work that the current routing layer can recognize: it must derive or sharply constrain the nucleus-generating substrate pre-nucleus dynamics, or prove a smaller benchmark-relevant collapse strictly inside that package, or otherwise discharge one of the benchmark derivation gates.
...
The active derivational program of \TheoryName{} now has an explicit safeguard against recursive depth drift. The benchmark exact-closure package is fixed, the same-package observer-side remainder gate has already been settled on the active completed branch, and the active-primary manuscript has already moved the live benchmark-facing burden below the proto-anchored exact-shell transport nucleus by deriving that nucleus from nucleus-generating substrate pre-nucleus dynamics. The next honest burden is therefore not ``something deeper'' in the abstract. It is either to derive or justify that pre-nucleus package from still more primitive substrate structure, or to prove a genuinely smaller theorem-level collapse inside it.
...
\begin{enumerate}
    ...
    \item The current active-primary frontier, namely the theorem that derives the proto-anchored exact-shell transport nucleus from nucleus-generating substrate pre-nucleus dynamics while preserving the same benchmark one-metric package.
\end{enumerate}
\end{definition}

\begin{remark}
These inputs already determine the current burden. They do not require a quantum ingredient by name. They require a benchmark-faithful derivational gain below the present pre-nucleus frontier.
\end{remark}
...
\begin{enumerate}
    \item It gives an explicit map from the quantum package \(\QuantumPackage\) to the current live pre-nucleus burden \(\PreNucPackage\), or to a strictly smaller benchmark-relevant datum inside that burden.
...
\begin{proof}
The benchmark package fixes the observer-accessible relativistic target, not the deepest admissible mathematical language of the substrate. Likewise, the current active-primary theorem fixes only that the present transport nucleus is generated from a deeper pre-nucleus package while preserving the same benchmark one-metric outputs. Neither statement requires the deepest substrate variables to be classical. Therefore an explicitly quantum package is not excluded in principle.
\end{proof}
...
At the current frontier, an explicitly quantum reformulation does not count as primary progress merely because it replaces classical notation by Hilbert-space or operator notation while preserving the same unresolved pre-nucleus burden.
\end{proposition}

\begin{proof}
The current routing rule already blocks a same-output deeper-origin relay from counting as new front-line progress when it preserves the same benchmark one-metric package and only relocates the unexplained substrate layer deeper. Replacing a classical relay by a quantum relay without deriving the current pre-nucleus package, collapsing it further, or discharging a benchmark derivation gate does not alter that logic. It changes the vocabulary of the deeper layer without changing the benchmark-facing burden. Therefore it does not by itself change project status.
\end{proof}
...
\begin{enumerate}
    \item it derives or justifies the nucleus-generating substrate pre-nucleus dynamics from a more primitive benchmark-faithful quantum package;
    \item it proves a genuinely smaller theorem-level benchmark-relevant collapse strictly inside the current pre-nucleus package;
...
\begin{proof}
The fixed routing inputs already determine that the current active-primary burden sits below the transport nucleus and inside or below the pre-nucleus package. A replacement branch must therefore do benchmark-facing work at that same burden or deeper. Item (1) does so directly by deriving the live pre-nucleus package from a more primitive quantum structure. Item (2) does so by proving that even the current pre-nucleus burden is not yet minimal. Item (3) covers the only other honest way a quantum branch could matter at current scope: it might discharge a still-open benchmark derivation gate by a route not already available on the active line. Absent one of those gains, the branch does not change the benchmark-facing burden and is therefore screened by the repository rule against recursive depth drift.
\end{proof}
...
Its present status is narrower. The benchmark package does not yet require a quantum ingredient by name, and the current routing layer does not license pausing the active primitive-reservoir main line merely because repeated depth shifts have become frustrating. The live burden remains what the current active-primary theorem says it is: derive or justify the nucleus-generating substrate pre-nucleus dynamics from still more primitive structure, or prove a sharper collapse inside that package. A quantum branch should therefore be opened, if desired, as a screened process answerable to that burden rather than as an automatic replacement for it.

That is the honest next burden. If a quantum manuscript can derive the live pre-nucleus package, collapse it further, or discharge a benchmark derivation gate in a new way, then it becomes benchmark-facing progress. If it cannot, then it is useful exploratory side work but not yet the default next main-line move.
