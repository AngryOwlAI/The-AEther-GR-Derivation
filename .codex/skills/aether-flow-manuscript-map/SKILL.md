...
4. Check whether the map points to a front-door file, benchmark gate, current frontier, direct support file, historical record, screened branch, or side continuation.
5. Only then open the manuscript itself for scientific detail.
...
- state whether it is front door, current frontier, direct support, historical support, screened branch, or side work
- if recommending a next step, tie it to the `active_primary` row or to a benchmark-gatekeeping row that explicitly reroutes the burden
